...
print_message $BLUE "=================================================="
print_message $BLUE "Hi-C接触衰减曲线分析工具 - E75阶段全量处理版"
print_message $BLUE "=================================================="
...
# 如果数据目录不存在，提示用户输入正确的路径
if [ ! -d "${DATA_ROOT}" ]; then
    print_message $YELLOW "数据目录不存在: ${DATA_ROOT}"
    print_message $YELLOW "请手动输入数据目录路径:"
    read -p "数据目录路径: " USER_DATA_ROOT
    
    # 验证用户输入的路径
    if [ -n "$USER_DATA_ROOT" ] && [ -d "$USER_DATA_ROOT" ]; then
        DATA_ROOT="$USER_DATA_ROOT"
        print_message $GREEN "使用用户指定的数据目录: ${DATA_ROOT}"
    else
        print_message $RED "错误：指定的路径无效或不存在"
        exit 1
    fi
fi
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            print_message $YELLOW "  --data-root PATH       数据根目录路径"
            print_message $YELLOW "  -h, --help             显示帮助信息"
            print_message $GREEN "\n示例:"
            print_message $YELLOW "  $0 --data-root /path/to/data  # 指定数据目录"
