...
#SBATCH -o ../logs/pairs2h5ad/allchr_pairs2h5ad.%j.out       ## Job's standard output and error file
#SBATCH -J allchr_pairs2h5ad              ## Job name
...
OUTPUT_DIR="/public/home/hpc254701055/Downloads/11pair2h5ad/output/h5ad_final_${RESOLUTION}"
...
do
    ...
    ${PYTHON_EXEC}  ../chrData_to_h5ad_code/oneByone_convert_intra_pairs_to_h5ad.py\
        ...
        -s ${CHR_SIZES} \
        -c ${CHR} \
        -o ${OUTPUT_DIR}/${CHR}.h5ad \
        -w ${WORKERS}\
        -r ${RESOLUTION}

    # 检查上一个命令是否成功
    if [ $? -ne 0 ]; then
        echo "错误: 处理 ${CHR} 时失败，脚本终止。"
        exit 1
    fi
done
