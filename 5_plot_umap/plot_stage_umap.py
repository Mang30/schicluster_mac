#!/usr/bin/env python3
"""
plot_stage_umap.py - 通用的Stage UMAP绘图脚本
支持绘制任意stage的UMAP图，并将结果保存到对应的stage文件夹中
"""
...
def plot_stage_umap(stage_name, h5ad_file=None, base_output_dir=None):
    """
    为指定stage绘制UMAP图
    """
    # 设置默认路径
    if h5ad_file is None:
        h5ad_file = f"/Volumes/SumSung500/CSU/0_HiRES/4_contact_decay_profile_2_h5ad/json2h5ad/output/{stage_name}.h5ad"
    ...
    try:
        ...
        if celltype_col:
            ...
            ax.set_title(f'{stage_name.upper()} - UMAP by Cell Type\\n({adata.shape[0]} cells)', fontsize=14)
        ...
        if stage_col:
            ...
            plt.title(f'{stage_name.upper()} - UMAP by Developmental Stage\\n({adata.shape[0]} cells)', fontsize=14)
            plt.tight_layout()
        ...
        if cellcycle_col:
            ...
            plt.title(f'{stage_name.upper()} - UMAP by Cell Cycle Phase\\n({adata.shape[0]} cells)', fontsize=14)
            plt.tight_layout()
        ...
        if numeric_features:
            ...
            plt.suptitle(f'{stage_name.upper()} - UMAP by Numeric Features', fontsize=16)
            plt.tight_layout()
        ...
        with open(summary_file, 'w') as f:
            f.write(f"{stage_name.upper()} 数据摘要\\n")
            f.write(f"=" * 50 + "\\n")
...
def main():
    ...
    主函数，支持命令行参数
    """
    parser = argparse.ArgumentParser(description='为指定stage绘制UMAP图')
    parser.add_argument('--stage', type=str, required=True,
                        help='Stage名称 (例如: stage_E75, stage_E70等)')
    parser.add_argument('--h5ad_file', type=str, required=False,
                        help='H5AD文件路径 (可选，默认根据stage自动构建)')
    parser.add_argument('--output_dir', type=str, required=False,
                        help='输出目录 (可选，默认为./output)')
    
    args = parser.parse_args()
    
    print(f"开始绘制{args.stage}的UMAP图...")
    
    success = plot_stage_umap(
    ...
    if success:
        print(f"\\n{args.stage} UMAP绘图任务完成!")
    else:
        print(f"\\n{args.stage} UMAP绘图任务失败!")
