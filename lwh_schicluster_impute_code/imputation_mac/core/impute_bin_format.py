...
def impute_bin_matrix(input_path, output_path, chrom_size_path, chrom, 
                     resolution=100000, use_mps=True):
    """
    ...
    # 读取染色体大小
    try:
        chrom_sizes = pd.read_csv(chrom_size_path, sep='\t', header=None, index_col=0).squeeze()
        n_bins = (chrom_sizes.loc[chrom] // resolution) + 1
        logging.info(f"   染色体 {chrom} 大小: {chrom_sizes.loc[chrom]:,} bp, bins: {n_bins}")
    except Exception as e:
        logging.error(f"   读取染色体大小文件失败: {e}")
        return False
    
    # 读取接触数据 (bin1 bin2 count)
    try:
        contacts = pd.read_csv(input_path, sep='\t', header=None, names=['bin1', 'bin2', 'count'])
        logging.info(f"   数据形状: {contacts.shape}")
        logging.info(f"   bin 范围: {contacts['bin1'].min()}-{contacts['bin1'].max()}, {contacts['bin2'].min()}-{contacts['bin2'].max()}")
    except Exception as e:
